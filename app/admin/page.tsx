...
export default function AdminPage() {
  ...
  async function sendReminders() {
    ...
    try {
      ...
      if (data.pending?.length === 0) {
        setReminderResult(
          "✅ Todos ya ordenaron, no hay recordatorios para enviar.",
        );
      } else {
        setReminderResult(
          `✅ Recordatorios enviados a: ${data.pending?.join(", ") ?? "nadie"}`,
        );
      }
    } catch {
  ...
  async function sendMagicLinks() {
    ...
    try {
      ...
      const data = await res.json();
      setLinksResult(
        `✅ Links enviados a ${data.sent} ${data.sent === 1 ? "persona" : "personas"}`,
      );
      await fetchOrders();
    } catch {
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
                ...
                <div key={item.id}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {itemOrders.map((o) => (
                      <span
                        key={o.name}
                        className="bg-gray-50 text-gray-500 text-sm px-3 py-1 rounded-full border border-gray-100"
                      >
                        {o.name}
                      </span>
                    ))}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-3">
          ...
          <button
            ...
            className="w-full bg-gray-900 hover:bg-gray-700 disabled:bg-gray-100 disabled:text-gray-300 text-white font-semibold rounded-xl py-3 transition-colors cursor-pointer"
          >
            {sendingLinks
              ? "Enviando links..."
              : "📨 Enviar links personalizados por DM"}
          </button>
          {linksResult && (
          ...
          <button
            ...
            disabled={reminding}
            className="w-full bg-blue-500 hover:bg-blue-400 disabled:opacity-50 text-white font-semibold rounded-xl py-3 transition-colors cursor-pointer"
          >
            {reminding
              ? "Enviando recordatorios..."
              : "📲 Recordar por DM a quienes faltan"}
          </button>
          {reminderResult && (
            <p className="text-sm text-gray-500 bg-gray-50 rounded-xl px-4 py-3">
              {reminderResult}
          ...
          <Link
            href="/summary"
            className="block w-full bg-green-500 hover:bg-green-400 text-white font-semibold rounded-xl py-3 text-center transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="w-full bg-red-500 border border-red-200 text-white hover:bg-red-400 disabled:opacity-40 font-semibold rounded-xl py-3 transition-colors cursor-pointer"
          >
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-gray-400 mb-4">
              Seleccioná a quién enviarle su link personal
            </p>

            {teamMembers.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-4">
                No hay miembros cargados en{" "}
                <code className="bg-gray-100 px-1 rounded">team:members</code>
              </p>
            ) : (
              <>
                <button
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-gray-400 mb-6">
              {CONFIRM_CONFIG[confirmAction].body}
            </p>
            <div className="flex gap-3">
              <button
